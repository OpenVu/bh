		...
		if module == "Blackhole.BhSettings":
			...
			if flash == True:
				if mounted == True:
					...
					if un_ver == bh_ver:
						from Blackhole.BhRed import BhRedPanel
						self.session.openWithCallback(self.menuClosed, BhRedPanel)
					else:
						from Blackhole.BhRed import BhRedWrong
						self.session.openWithCallback(self.menuClosed, BhRedWrong)
				else:
					from Blackhole.BhRed import BhRedDisabled
